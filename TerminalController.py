...
class TerminalController:
    ...
    def __init__(self, term_stream=sys.stdout):
        ...
        self.COLS ,self.LINES = self.size()
    ...
    def cols( self, cols, divider='|' ):
        import textwrap
      
        #calculating column width
        count = len(cols)
        self.COLS , self.LINES = self.size() # refresh sizes
        dcount = count - 1

        width = ( self.COLS - ( dcount * len(divider) ) ) / count
        print "cols:%d,count:%s,dc:%d,l:%d" \
            %( self.COLS,count , dcount, len(divider))

        wrapper = textwrap.TextWrapper(width=width)
        
        columns = [] 
        for c in cols:
            columns += ( wrapper.wrap( c ) , )
